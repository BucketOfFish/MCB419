...
class Salience():
    def __init__(self):
        self.net = Salience_Net()
        self.optimizer = optim.Adam(self.net.parameters(), lr=learning_rate, weight_decay=weight_decay)
        self.lossFunction = nn.MSELoss()
    def eval(self, state, action, quality, t_back):
        return self.net(Variable(torch.FloatTensor(state)), Variable(torch.FloatTensor([action]), Variable(torch.FloatTensor([quality]), Variable(torch.FloatTensor([t_back])))
    def train(self, state, action, quality, t_back, salience):
        self.net.train()
        self.optimizer.zero_grad()
        predicted_salience = self.eval(state, action, quality, t_back)
        salience = Variable(torch.FloatTensor([salience]))
        salience.requires_grad = False
        loss = self.lossFunction(predicted_salience, salience)
        loss.backward()
        self.optimizer.step()
        return loss.data[0]
    
salience_function = Salience()
def S(state, action, Q(state, action), t_back):
    return salience_function.eval(state, action, Q(state, action), t_back).data[0]
